...
const checkout = async () => {
    try {
        ...
        const res = await $fetch('https://m-markly.netlify.app/.netlify/functions/create-checkout-session', {
            method: 'POST',
...
<template>
    <section class="text-white px-6 pb-5 pt-20">
        <section class="max-w-4xl mx-auto p-6 rounded-xl shadow-lg mt-10">
            <section v-if="emptyCart===true" class="text-red-700 text-2xl mb-10">
                Cart is empty
            </section>
            <section v-else-if="emptyInputs===true" class="text-red-700 text-2xl mb-10">
                Please fill inputs
            </section>
            <h1 class="text-3xl font-bold mb-6">Checkout</h1>
            <section class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                <input v-model="name" type="text" placeholder="Full Name" required
                    class="border bg-[#1A1A1A] p-3 rounded w-full focus:bg-black" />
                <input v-model="email" type="email" placeholder="Email"
                    class="border bg-[#1A1A1A] p-3 rounded w-full" required/>
                <input v-model="address" type="text" placeholder="Address" required
                    class="border bg-[#1A1A1A] p-3 rounded w-full md:col-span-2" />
                <input v-model="city" type="text" placeholder="City" class="border bg-[#1A1A1A] p-3 rounded w-full" required/>
                <input v-model="zip" type="text" placeholder="ZIP Code" required
                    class="border bg-[#1A1A1A] p-3 rounded w-full" />
            </section>
            <section class="mb-6">
